"""Bronze -> Gold dimension and fact transforms (`dim_*`/`fct_*`), one season at a time.

Purpose
-------
Every function here reads `raw_*` rows from Bronze and upserts the
corresponding `dim_*`/`fct_*` rows into Gold. All of them are private and
only ever correct when called in the fixed order `run_transform`
(`transformers/bronze_to_gold.py`) calls them in — nothing outside that
orchestration should call these directly. Split out of a single
1,430-line `bronze_to_gold.py` (Phase 7 audit, Medium) alongside
`transformers/finalize.py` (the two post-hoc "finalize" passes) and
`transformers/marts.py` (the four mart-builders) — each concern is now
independently readable and testable, re-exported through `run_transform`.
...
def _transform_drivers(raw_engine: Engine, gold_engine: Engine, *, pipeline_version: str) -> int:
    ...
    intentionally left `None` here — see `transformers.finalize`'s
    `_finalize_driver_world_titles`, which sets it in a separate pass after
    `dim_season` exists, using `update_columns` so this function re-running
    later never wipes that finalized value back out (see `utils.db.upsert`'s
...
def _transform_season_shell(
    ...
    see `transformers.finalize._finalize_season_champion`, which sets those
    in a later pass, and `utils.db.upsert`'s `update_columns` docstring for
    why this function must not overwrite them back to `NULL` on a later
    re-run.
